(function () {
  ...
  function payWhish(v) {
    ...
    // QR image is generated by a public service; if it can't load we hide it and the number + Copy button still work.
    const qrSrc = "https://api.qrserver.com/v1/create-qr-code/?size=220x220&margin=10&data=" +
      encodeURIComponent(STORE.whishNumber);
    openModal(modalHead("Pay with Whish", v.title) +
      `<div class="modal-body">
        ...
        <div class="qr-wrap">
          <img class="qr-img" alt="QR code for the Whish number ${esc(STORE.whishNumber)}" src="${esc(qrSrc)}"
               onerror="this.closest('.qr-wrap').style.display='none'">
          <span class="qr-cap">Scan to get the Whish number</span>
        </div>
